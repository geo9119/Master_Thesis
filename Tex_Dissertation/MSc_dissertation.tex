...
of them were empoyed and used and compaired: the \textit{Boosted Descision Trees} (BDT) and the 
...
momenta, missing energy, etc. Then we sort all events by each variable and we choose the one that gives the
best separation between signal and background. This is how the first node is made. Then, in the two branches 
...
In the physical case that I examine I have what will later be denoted as SUSY signal: corresponding to the 
...
\begin{figure}[ht!]
  ...
  \caption{The green line represents the statistical significance $\frac{S}{\sqrt{S+B}}$ where as it can be
 ...
 \includegraphics[width=0.6\linewidth]{{images/BDT_output}.pdf}
  \caption{The BDT output for the signal and the two backgrounds. The y axis has a log scale.}
\end{figure}
...
The following histograms show the aforementioned variables as they were given for training and testing.
