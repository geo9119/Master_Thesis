...
In the first stage it will focus on prescision standard model physics such as Higgs and top quark measurements and in the two subsequent, among others, there will be searches
...
to discriminate between signal and background with the goal to measure the top squark
...
Furthermore, the experiment has two detector concepts: CLIC SiD and CLIC ILD. 
Both have been  developed for the International Linear Collinder but it has been
...
The model that describes elementary particle physics is the Standard Model~\cite{cheng1984gauge}~\cite{weinberg1967model}.
It involves matter fields (fermions)
and vector gauge bosons (force carriers) that they stem from the fact that the SM Lagrangian is subject to $SU(3) \otimes 
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.6\linewidth]{{images/ATLAS_SUSY_Stop_tLSP}.png}
  \label{fig2.15}
  \caption{\scriptsize{Mass exclusion limits at stop-neutralino plane (combined summary plot from ATLAS 
  twiki).}}
\end{figure}

%Whereas in the figure 3.2 there are the latest exclusion limits in ATLAS for p-p collisions at 13 TeV in the gluino-
%stop plane.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.6\linewidth, scale=5]{{images/gluino}.png}
  \label{}
  \caption{\scriptsize{Mass exclusion limits at stop-gluino plane~\cite{atlassearch}.}}
\end{figure}
...
measurement of the stop quark mass at the CLIC experiment.

I investigate the production of a pair of top squarks at $\surd s$ = 3 TeV and assuming an  integrated 
...
and for this model the mass of the top squark is $m_{\tilde{t}}$ = 845 GeV (denoted as $M_{nom}$ later
in the text)
whereas the mass of the 
lightest neutralino is $m_{\tilde{\chi}_{1}^{0}}$= 357 GeV. This is not the only possible decay of a top 
squark but it is the
...
In this chapter I describe the procedure that I followed until the measurement of the top squark mass. 
...
As shown in the following graph, the value of the BDT cut that was chosen is $cut_{BDT}=-0.036$ due to the 
...
\begin{figure}[ht!]
 \centering
  ...
  seen it reaches the greatest value at $\num{-0.0238}$. 
  The cut is also known as descision boundary and defines the
  critical region.}}
...
\begin{enumerate}
 \item $\theta_{miss}$
 \item missing $p_{T}$ 
 \item $\Delta \phi$ as the angle between the $t\bar{t}$
 \item $cos\theta_{top}$
\end{enumerate}
...
\begin{figure}[ht!]
  ...
  \caption{\scriptsize{The energy of the reconstructed top quark candidate.}}
\end{figure}


\begin{figure}[ht!]
  \includegraphics[width=0.7\linewidth]{{images/ratio_plot_witherrors}.pdf}
  \caption{\scriptsize{The signal efficiency with the associated errors.}}
\end{figure}
...
\subsubsection{Mass Measurements Results}
...
$i$ of the histogram. A $\chi^{2}$ was calculated for each different template, summarised in the table below.
...
The mass was found  to be $m_{\tilde{t}}$ = 839 GeV compaired to the mass of the Monte Carlo sample which 
is $m_{\tilde{t}}$ = 845 GeV/$c^{2}$.
...
\centering
  ...
  \caption{\scriptsize{The mass result after 5000 different measurements. 
 A toy Monte Carlo method was used that yielded an uncertainty of 11 GeV.}}
\end{figure}
...
The nominal mass is $M_{nom}$= 845 GeV/$c^{2}$, my measurement is one sigma  inside this range,
...
\begin{equation}
 m_{\tilde{t}}=811\pm 15 \quad \textrm{GeV/}c^{2}
\end{equation}
...
GBDT         &       7.68      &  811 $\pm$ 15 GeV/$c^{2}$      &  455		\\
\hline
 %inserts single line
\end{tabular}
\label{table:nonlin} % is used to refer this table in the text
...
(for the mass and the uncertainty calculation).
...
Similar contradiction produces the careful examination of the signal efficiency plots(4.8 for BDT and 4.20
...
Another interesting observation is the dependence of the discrimination strength of the classifiers with respect to 
...
something that the experimental community knew. 
...
\begin{itemize}
 \item As I investigated the following channel
    \begin{equation}
      ...
      \rightarrow W^{+}W^{-}b\bar{b} \tilde{\chi}_{1}^{0} \tilde{\chi}_{1}^{0}
    \end{equation}
the decay of the W boson can be hadronic or leptonic. Isolating and investigating 
the cases that $W\rightarrow l\nu_{l}$
,i.e. the leptonic channel a mass measurement can be achieved. 
...
the possible stop decays denoted in page 11. In that case, since there will be no SUSY background
and work has to be done in order to simulate a new one involving other interactions.
