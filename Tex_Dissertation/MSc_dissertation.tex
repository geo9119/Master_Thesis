...
\begin{figure}[ht!]
  \includegraphics[width=0.6\linewidth]{{images/cut_efficiency}.pdf}
  \caption{\scriptsize{The green line represents the statistical significance $\frac{S}{\sqrt{S+B}}$ where as it can be
  seen it reaches the greatest value at -0.036. The cut is also known as descision boundary and defines the
  critical region.}}
  
 \includegraphics[width=0.6\linewidth]{{images/BDT_output}.pdf}
...
\begin{figure}[ht!]
  \includegraphics[width=0.7\linewidth]{{images/postbdtcut}.pdf}
...
\hline %inserts double horizontal lines
Before TMVA (Number of events)  & After TMVA (Number of events) & $\%$  \\ \hline % inserts single horizontal line
3005   & 542    & 18 \\
17628  & 567    &  3 \\
113494 &  3036  &  2 \\  
\hline
...
The mass was found  to be $m_{\tilde{t}}$ = 860 GeV.
...
\centering
  ...
  \caption{\scriptsize{Toy Monte carlo method that yielded an uncertainty of 14 GeV.}}
\end{figure}
...
\begin{equation}
 m_{\tilde{t}}=860\pm 14 \textrm{GeV}/c^{2}
\end{equation}

Although the nominal mass is $M_{nom}$= 846 GeV/$c^{2}$, my calculation is one sigma  inside this range,
...
In the first phase -training phase- a rough approximation for the prediction model is made. Subsequently, the error of the 
the error of the model and the actual values expected, is calculated (residual) and it is added in the
previous model making it a slightly  better compaired to the previous one. 
This procedure is repeated various times. 
...
both BDT's and GBDT's lies beyond the scope of this project but it can be seen in~\cite{schapire1999brief},
